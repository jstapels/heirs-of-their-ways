...
async function updateModuleUrls(version) {
  ...
  const repo = moduleData.url || "https://github.com/jstapels/heirs-of-their-ways";
  ...
  await fsp.writeFile(MODULE_JSON, JSON.stringify(moduleData, null, 2) + "\n");
...
async function createReleaseZip(version) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (fs.existsSync("packs")) {
      const packDirs = fs.readdirSync("packs", { withFileTypes: true })
        .filter(entry => entry.isDirectory() && !entry.name.startsWith("_"))
        .map(entry => entry.name);

      for (const packDir of packDirs) {
        archive.directory(`packs/${packDir}`, `${MODULE_ID}/packs/${packDir}`);
      }
    }
...
async function main() {
  try {
    ...
    await updateModuleUrls(version);
